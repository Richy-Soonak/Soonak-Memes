      ...
      <nav aria-label="Progress">
        <ol className={cn("flex flex-col gap-y-3", colors, className)}>
          {steps?.map((step, stepIdx) => {
            ...
            return (
              <LazyMotion features={domAnimation} key={stepIdx}>
                <m.li
                ...
                >
                  <div className="flex w-full max-w-full items-center">
                    <button
                      ...
                      className={cn(
                        "flex w-full items-center justify-center gap-x-4 rounded-large px-3 py-2.5",
                        status === "active" ? "cursor-pointer" : "cursor-not-allowed"
                      )}
